...
* Para instalar los paquetes "foreign", "forecast", "gWidgets", utilizamos:
```{r, eval=FALSE}
...
* Para cargar un paquete O varios paquetes nuevos a la vez, al área de trabajo
utilizamos:
```{r, eval=TRUE}
library("devtools")
```
...
# El operador `:` se usa para generar secuencias enteras
str(x)
is.atomic(x)
```
...
## <b> <font color=SteelBlue face=Cambria size=7> Atributos </font> </b>

* Los objetos en R poseen varios atributos tales como:
    + nombres
    + clase
    + dimensiones, etc
* La función `attributes()` muestra los atributos de un objeto.

```{r}
x <- c(1:5)
attributes(x)
x <- list(nomb_obj1=1:3, nomb_obj2=c("Source", "Stat", "Lab"), nomb_obj3=c(TRUE, FALSE))
attributes(x)
```
 
## <b> <font color=SteelBlue face=Cambria size=7> Matrices </font> </b>
* Una matriz es un vector con el atributo `dimensión`
* La `dimensión` es un vector de longitud 2: `c(nrow, ncol)`

```{r}
A <- matrix (c(1:9),nrow=3, ncol=3, byrow=FALSE)
...
```
## <b> <font color=SteelBlue face=Cambria size=7> Matrices </font> </b>
* `dim()` se usa para añadir el atributo `dimensión` a un vector, o
...
## <b> <font color=SteelBlue face=Cambria size=7> Accediendo a elementos </font> </b>
```{r}
(A <- matrix (c(1:9),nrow=3, ncol=3, byrow=FALSE))
A[1,2] # componente
A[,3]  # columna
...
## <b> <font color=SteelBlue face=Cambria size=7> Factores </font> </b>
* Se usan para representar variables categóricas. Es mejor utilizar factores con etiquetas, que vectores enteros.
    + "Femenino" y "Masculino" que 0 y 1 
```{r}
x <- factor(x=c(0,1,1,0,1,0,1), levels=c(0,1), labels = c("Femenino", "Masculino"))
x
attributes(x)
```
...
# frecuencias
table(x)
# para considerar las etiquetas
unclass(x)
```

## <b> <font color=SteelBlue face=Cambria size=7> Data Frame </font> </b>
* Propiedades:
    + Es una lista en la que todos los elementos tienen la misma longitud
    ...
    + Presentan varios atributos adicionales `class`, `rownames`, `names`, etc

* R dentro de sus bases de datos posee un sin número de data frames, por ejemplo:
```{r}
data(mtcars)
...
## <b> <font color=SteelBlue face=Cambria size=7> Data Frame </font> </b>
```{r}
# atributos names, rownames, class
attributes(mtcars)
```
...
colnames(mtcars) # nombres de columnas
# nombres filas: rownames()
```
...
## <b> <font color=SteelBlue face=Cambria size=7> Missing Values </font> </b>
* Los valores pérdidos en R se denotan por `NA`, `NaN`.
* `NA` Not Available. Dato pérdido.
* `NaN` Not a Number. Resultado de una indeterminación.
    + `NaN` es `NA`, el reciproco no se cumple.
...
* is.na(). prueba si un objeto es NA.
* is.nan(). prueba si un objeto es NaN.
```{r}
x <- c(Inf*0, 3.2, NA, 5, NA)
is.na(x)
```
...
## <b> <font color=SteelBlue face=Cambria size=7> Sentencia if </font> </b>
* Se utiliza para testear una proposición y realizar una determinada operacion
...
## <b> <font color=SteelBlue face=Cambria size=7> Sentencia for </font> </b>
* Se utiliza para testear una proposición y realizar una determinada operación
...
## <b> <font color=SteelBlue face=Cambria size=7> Subsetting </font> </b>
* la función `subset()` se utiliza para seleccionar elementos de un objeto.

```{r}
x <- 1:5
y <- subset(x,subset = x>3)
y <- x[x>3]
y
```
...
```{r}
table(mtcars[,"cyl"])
data <- subset(mtcars, subset = mtcars[,"cyl"]!=6)
# data <- subset(mtcars, subset = mtcars[,3]!=6)
table(data[,"cyl"])
```


## <b> <font color=SteelBlue face=Cambria size=7> DIAPOSITIVA PRUEBA </font> </b>
```{r}
table(mtcars[,"cyl"])
data <- subset(mtcars, subset = mtcars[,"cyl"]!=6)
# data <- subset(mtcars, subset = mtcars[,3]!=6)
